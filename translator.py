...
def main() -> None:
    """
    CLI entry point for translating PDF documents using DeepSeek, Anthropic, OpenAI, or Gemini.
    
    Steps:
      1) Validate file format.
      2) Convert to PDF (if necessary).
      3) Extract text from the PDF using the advanced conversion (convert_pdf_to_text).
      4) Split the resulting text into pages (using the inserted page-break markers).
      5) Split each page into chunks.
      6) Translate all chunks in parallel.
      7) Reassemble and save the final translation.
      8) Calculate usage and cost.
    """
    parser = argparse.ArgumentParser(
        description="Translate PDF documents via DeepSeek, Anthropic, OpenAI, or Gemini (with chunk-level concurrency)."
    )
    parser.add_argument(
        "--file",
        required=True,
        help="Path to the input file (pdf/doc/docx/odt).",
    )
    parser.add_argument(
        "--target",
        required=True,
    ...
    parser.add_argument(
        ...
        help="API provider for translation.",
    )
    parser.add_argument(
        "--model",
        default=None,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2000,
        help="Approximate maximum tokens allowed per chunk. Default=2000.",
    )
    parser.add_argument(
        "--concurrency",
        type=int,
        default=4,
        help="Number of parallel translation requests for chunks. Default=4.",
    )
    parser.add_argument(
        "--trace",
        action="store_true",
    ...
    try:
        # 1) Validate and convert to PDF if needed
        validate_file(file_path, logger=logger)
        pdf_path = convert_to_pdf(file_path, logger=logger)

        # 2) Extract text from the PDF using the advanced conversion function.
        extracted_text = convert_pdf_to_text(pdf_path, logger=logger)
        # Split the text into pages based on the inserted marker.
        page_texts = [page.strip() for page in extracted_text.split("<<<PAGE_BREAK>>>") if page.strip()]
        num_pages = len(page_texts)
        if num_pages == 0:
            raise ValueError(f"'{pdf_path.name}' contains no extractable text.")

        logger.info(f"Advanced extraction resulted in {num_pages} page(s).")
        ...
        logger.info(
            f"Starting translation of {num_pages} page(s) with concurrency={args.concurrency} and max_chunk_tokens={args.max_chunk_tokens}."
        )
        ...
        # 5) Reassemble the final translated text and save to file
        ...
        # 6) Calculate usage and cost (DeepSeek only)
        if args.provider == "deepseek":
            ...
            logger.info("Cost estimation is available only for DeepSeek.")
...
# # ------------------------------------------------------------------------
# # PDF -> Text Extraction (per page in memory)
# # ------------------------------------------------------------------------
# def extract_all_page_texts(pdf_path: Path, logger: logging.Logger) -> List[str]:
#     """
#     Extracts the text content from each page of a PDF, returning a list of strings (one per page).
...
#     Returns:
#         List[str]: A list of strings for each page (even if empty).
#     """
...
#     # pdfplumber often extracts text better than PyPDF2
...
#     # System instruction to guide the academic translator
#     system_prompt = (
#         "You are an elite academic translator with expertise in scientific and technical documentation, committed to preserving the integrity of academic works while ensuring precise translation.\n\n"
...
#         "ACADEMIC CONVENTIONS:\n"
#         "1. Terminology:\n"
#         "   - Use field-appropriate technical vocabulary and maintain term consistency, standard abbreviations, and measurement units\n"
#         "2. Style:\n"
#         "   - Maintain academic register, formality, hedging language, passive voice (if present), and citation styles\n"
#         "3. Field-Specific:\n"
#         "   - Respect discipline-specific conventions and preserve specialized notation systems\n\n"
        
#         "QUALITY CONTROL:\n"
#         "1. Consistency:\n"
#         "   - Ensure uniform terminology and consistent formatting throughout the translation\n"
#         "2. Accuracy:\n"
#         "   - Preserve technical precision, numerical accuracy, and methodological clarity\n\n"
        
#         "OUTPUT REQUIREMENTS:\n"
#         "- Provide only the translated content without any explanations, comments, or extra text\n"
#         "- Do not include any part of the original text unless explicitly specified\n"
#         "- Ensure all formatting and non-translatable segments are preserved exactly\n"
#         "- If any ambiguity arises, preserve the original term or structure without translation\n"
#         "- [NO-TRANSLATE] ... [/NO-TRANSLATE] markers may be used to explicitly delimit non-translatable content\n\n"
        
#         "Final Check:\n"
#         "Before outputting the final translation, perform an internal verification to ensure that all rules and formatting have been strictly followed."
#     )
...
#         translated_text = response.text
#         usage_info = {
#             "prompt_tokens": 0,  # The Gemini API does not return token counts
#             "completion_tokens": 0
#         }
#     else:
#         raise ValueError(f"Unsupported provider: {provider}")
...
# ) -> List[List[str]]:
#     """
#     Splits the text from each PDF page into chunks if needed.
    
#     Args:
#         page_texts (List[str]): Raw text of each PDF page.
#         max_chunk_tokens (int): Approximate token limit per chunk.
#         logger (logging.Logger): For debug messages.
...
#       4) Split each page into chunks.
#       5) Translate all chunks in parallel.
#       6) Reassemble and save the final translation.
#       7) Calculate usage and cost.
#     """
#     parser = argparse.ArgumentParser(
#         description="Translate PDF documents via DeepSeek, Anthropic, OpenAI, or Gemini (with chunk-level concurrency)."
#     )
